...
%data sequence 
% PRBS9 sps=6 Bitrate 20Gbps
%Hence bit slot time is 0.05ns
%Sampling frequency is fixed as sps*baudrate = 120GHz.
...
%Calculating signal output from photodetector

N_sig = length(Efib_out);
Vout_rx = zeros(N_sig,1);
%Averaging the output calculated
iter = 5;
for i=1:iter
Vout_rx =  Vout_rx+PD(Efib_out, Rd, PD_bw, fs_lw, RL,att); %Noisy volt signal
end
Vout_rx = Vout_rx/iter;
%Plotting voltage constellation output
%Choosing the middle sample in every symbol slot.
V_rx_constt = Vout_rx(floor(sps/2):sps:end);
V_rx_constt = V_rx_constt/(max(abs(V_rx_constt)));
scatterplot(V_rx_constt)
title('Voltage Constellation output with a Photodiode of RL = 50 ohms')
grid on

%Finding thoeretical avg. SNR for the PD
SNR =  ((Rd*mean(Pfib_out)))^2 /( 2*q*mean(Rd*Pfib_out)*PD_bw +4*k*T*Fn*PD_bw/(RL) );
