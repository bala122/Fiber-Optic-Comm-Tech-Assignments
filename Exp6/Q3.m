...
%Q3
%PD load resistance 10Kohms case

%Generating OOK mod. at 20Gbaud 
sps = 8;
att= 60;%Attenuation in stopband LPF
...
%data sequence 
% PRBS9 sps=8 Bitrate 20Gbps
%Hence bit slot time is 0.05ns
%Sampling frequency is fixed as sps*baudrate = 160GHz.
...
%Finding output voltage signal from the PD
...
%Constellation plot
%Choosing samples at the middle of the symbol time slot
V_rx_constt = Vout_rx(floor(sps/2):sps:end);
V_rx_constt = V_rx_constt/(max(abs(V_rx_constt)));
scatterplot(V_rx_constt)
title('Voltage Constellation output with a Photodiode of RL = 10 Kohms')
grid on
